...
def main():
    data_path = os.path.join("data", "amazon_fashion_clean.jsonl")
    data_loader = AmazonDocumentLoader(data_path)

    for batch in tqdm(
        data_loader.stream_from_disk(batch_size=CLIENT_BATCH_SIZE),
        total=data_loader.batch_len(CLIENT_BATCH_SIZE),
    ):
